...
class VatRequest extends FormRequest
{
    ...
    public function rules()
    {
        if ($this->isMethod('POST')) {
            return [
                'name_ar'        => ['required', 'string', 'max:255', Rule::unique('vats')
                    ->where(fn ($query) => $query->where('user_id', Auth::guard('api')->user()->id))],
                'name_en'        => ['required', 'string', 'max:255' , Rule::unique('vats')
                    ->where(fn ($query) => $query->where('user_id', Auth::guard('api')->user()->id))],
                'value'          => ['required', 'numeric'],
            ];
        } elseif ($this->isMethod('PUT') || $this->isMethod('PATCH')) {
            return [
                'name_ar'        => ['sometimes', 'string', 'max:255', Rule::unique('vats')
                    ->where(fn ($query) => $query->where('user_id', Auth::guard('api')->user()->id))
                    ->ignore($this->route('vat')->id)],
                'name_en'        => ['sometimes', 'string', 'max:255', Rule::unique('vats')
                    ->where(fn ($query) => $query->where('user_id', Auth::guard('api')->user()->id))
                    ->ignore($this->route('vat')->id)],
    ...
    public function messages()
    {
        if ($this->header('Accept-Language') == 'ar'){
            return [
                ...
                'value.required'   => 'قيمة الدفعة مطلوبة',
                'value.numeric'    => 'قيمة الدفعة يجب ان تكون رقم',
            ];
        }else{
            return [
                ...
                'value.required'   => 'Payment Method Value is required',
                'value.numeric'    => 'Payment Method Value must be a number',
            ];
        }
    }
}
